...
class EquaPipeline:
    ...
    # Database setup
    conn = sqlite3.connect('equa.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `equa`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM equa WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO equa (date, title, link, content)"
